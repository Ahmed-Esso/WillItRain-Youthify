...
@op(out=DynamicOut())
def search_t2mwet_files(context):
    """البحث عن ملفات T2MWET في MERRA-2 لسنة 2022 للإسكندرية"""
...
@op
def process_t2mwet_file(context, granule):
    """معالجة ملف واحد واستخراج T2MWET للإسكندرية"""
    try:
        ...
        # ✅ تحويل من كلفن إلى مئوية (لأنه متغير حراري)
...
@op
def transform_t2mwet_daily(context, df: pd.DataFrame):
    """حساب الإحصائيات اليومية لـ T2MWET"""
    if df.empty:
        return pd.DataFrame()
...
@op
def load_t2mwet_to_snowflake(context, df: pd.DataFrame):
    ...
    conn.commit()
    cur.close()
    context.log.info(f"✅ تم تحميل {len(df)} يوم لـ T2MWET")

@job
def t2mwet_job():
    """Job كامل لمعالجة T2MWET للإسكندرية 2022"""
    files = search_t2mwet_files()
    processed = files.map(process_t2mwet_file)
    transformed = processed.map(transform_t2mwet_daily)
    transformed.map(load_t2mwet_to_snowflake)
