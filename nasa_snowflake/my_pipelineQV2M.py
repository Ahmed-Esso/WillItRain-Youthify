...
def process_single_file(context, granule) -> pd.DataFrame:
    """
    معالجة ملف واحد وحساب المتوسط اليومي للرطوبة
    """
    try:
        ...
        context.log.info(f"✅ Processed {len(combined)} daily humidity records from file")
...
def transform_daily_data(context, df: pd.DataFrame) -> pd.DataFrame:
    ...
    context.log.info(f"🔄 Transforming {len(df)} daily humidity records...")
    ...
    daily_summary = (
        ...
        .agg({
            list(df.columns)[3]: 'mean',  # أول عمود بيانات (QV2M)
    ...
    context.log.info(f"✅ Transformed to {len(result)} daily humidity summary records")
...
def load_daily_to_snowflake(context, df: pd.DataFrame):
    """
    تحميل بيانات الرطوبة اليومية لـ Snowflake
    """
    if df.empty:
        context.log.warning("⚠️ Empty dataframe - skipping load")
        return "skipped"
    
    context.log.info(f"📤 Loading {len(df)} daily humidity records to Snowflake...")
    ...
    try:
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS NASA_DAILY_HUMIDITY (
                date DATE,
                year INT,
                month INT,
                day INT,
        ...
        insert_query = """
            INSERT INTO NASA_DAILY_HUMIDITY 
        ...
        context.log.info(f"✅ Successfully loaded {len(df)} daily humidity records")
...
@job
def nasa_daily_humidity_pipeline():
    """
    Pipeline لمعالجة بيانات الرطوبة اليومية (QV2M)
    """
    # بحث عن الملفات
...
if __name__ == "__main__":
    ...
    result = execute_pipeline(nasa_daily_humidity_pipeline)
    print(f"✅ Humidity pipeline finished: {result.success}")
