...
def process_file_tox(context, granule):
    try:
        ...
        if VARIABLE not in ds:
            ...
            alternatives = ["O3", "OZONE", "TO3", "OZONE_TOTAL"]
        ...
        # If TOX is found, process normally
...
def load_to_snowflake_tox(context, df):
    if df.empty: 
        context.log.info("No TOX data to load")
        return
    ...
    try:
        ...
        conn.commit()
        context.log.info(f"Successfully loaded {len(rows)} TOX rows into {table_name}")
        
    except Exception as e:
        context.log.error(f"Error loading TOX data to Snowflake: {e}")
        conn.rollback()
        raise e
    finally:
        cur.close()
