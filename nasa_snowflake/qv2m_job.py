...
def process_file_qv2m(context, granule):
    try:
        ...
        if VARIABLE not in ds:
            ...
            # Try common alternative names for specific humidity
            alternatives = ["Q2M", "SH", "SPECIFIC_HUMIDITY", "HUMIDITY"]
            ...
            context.log.info("No specific humidity variable found, returning empty DataFrame")
            return pd.DataFrame()
        
        # If QV2M is found, process normally
        ...
        # QV2M is specific humidity at 2 meters, usually in kg/kg
        # Can convert to g/kg if needed: df[VARIABLE] = df[VARIABLE] * 1000
        
        ds.close()
        context.log.info(f"Processed {len(df)} QV2M data points")
