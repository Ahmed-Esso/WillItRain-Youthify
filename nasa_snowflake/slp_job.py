...
@op(out=DynamicOut(), name="search_files_slp")
def search_files_slp(context):
...
@op(name="process_file_slp")
def process_file_slp(context, granule):
    try:
        ...
        if VARIABLE not in ds:
            ...
            # Try common alternative names for sea level pressure
            alternatives = ["MSL", "MSLP", "PRESSURE_MSL", "SEA_LEVEL_PRESSURE"]
            ...
            context.log.info("No sea level pressure variable found, returning empty DataFrame")
            return pd.DataFrame()
        
        # If SLP is found, process normally
        ...
        # SLP is sea level pressure, usually in Pa
        # Convert to hPa if needed: df[VARIABLE] = df[VARIABLE] / 100
        
        ds.close()
        context.log.info(f"Processed {len(df)} SLP data points")
        return df[["time", VARIABLE]]
    except Exception as e:
        context.log.error(f"Error processing SLP file: {e}")
        return pd.DataFrame()

@op(name="transform_daily_slp")
def transform_daily_slp(context, df):
    if df.empty: 
        context.log.info("No data to transform for SLP")
        return pd.DataFrame()
...
@op(name="load_to_snowflake_slp")
def load_to_snowflake_slp(context, df):
    if df.empty: 
        context.log.info("No SLP data to load")
        return
    ...
    try:
        ...
        conn.commit()
        context.log.info(f"Successfully loaded {len(rows)} SLP rows into {table_name}")
        
    except Exception as e:
        context.log.error(f"Error loading SLP data to Snowflake: {e}")
        conn.rollback()
        raise e
    finally:
        cur.close()
...
@job
def slp_job():
    files = search_files_slp()
    processed = files.map(process_file_slp)
    transformed = processed.map(transform_daily_slp)
    transformed.map(load_to_snowflake_slp)
