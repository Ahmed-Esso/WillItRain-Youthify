...
VARIABLES = ["TOX"]  # تركيز الأوزون الكلي
...
# ==========================
# DAGSTER OPS - DAILY AVERAGE FOR TOX
# ==========================

@op(out=DynamicOut())
def search_nasa_files_tox_2022(context):
    """
...
@op
def process_single_file_tox(context, granule) -> pd.DataFrame:
    """معالجة ملف واحد وحساب المتوسط اليومي لـ TOX"""
    try:
        ...
        if not all_daily_data:
            context.log.warning(f"⚠️ No TOX variable found in file")
            return pd.DataFrame()
        
        combined = pd.concat(all_daily_data, ignore_index=True)
        context.log.info(f"✅ Processed {len(combined)} daily TOX records from file")
        return combined
...
@op
def transform_daily_tox(context, df: pd.DataFrame) -> pd.DataFrame:
    """تحويل البيانات اليومية لـ TOX"""
    if df.empty:
        return df
    
    context.log.info(f"🔄 Transforming {len(df)} daily TOX records...")
    ...
    daily_summary = (
        ...
        .agg({'TOX': 'mean', 'lat': 'count'})
        .reset_index()
    )
    
    daily_summary.rename(columns={'TOX': 'avg_total_ozone', 'lat': 'measurement_count'}, inplace=True)
    ...
    daily_summary["ozone_category"] = daily_summary["avg_total_ozone"].apply(get_ozone_category)
    ...
    daily_stats.columns = ['date', 'max_total_ozone', 'min_total_ozone', 'ozone_std']
    
    final_result = pd.merge(daily_summary, daily_stats, on="date", how="left")
    
    result = final_result[[
        "date", "year", "month", "day", "day_of_year", "day_name", "season", "variable", 
        "avg_total_ozone", "max_total_ozone", "min_total_ozone", "ozone_std",
        "ozone_category", "measurement_count"
    ]]
    
    context.log.info(f"✅ Transformed to {len(result)} daily TOX summary records for Egypt")
    return result

@op
def load_daily_tox_to_snowflake(context, df: pd.DataFrame):
    """تحميل بيانات TOX لـ Snowflake"""
    if df.empty:
        context.log.warning("⚠️ Empty dataframe - skipping load")
        return "skipped"
    
    context.log.info(f"📤 Loading {len(df)} daily TOX records to Snowflake...")
    ...
    try:
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS NASA_DAILY_TOTAL_OZONE (
                date DATE, year INT, month INT, day INT, day_of_year INT,
                day_name STRING, season STRING, variable STRING,
                avg_total_ozone FLOAT, max_total_ozone FLOAT,
                min_total_ozone FLOAT, ozone_std FLOAT,
        ...
        insert_query = """
            INSERT INTO NASA_DAILY_TOTAL_OZONE 
            (date, year, month, day, day_of_year, day_name, season, variable, 
             avg_total_ozone, max_total_ozone, min_total_ozone, ozone_std,
             ozone_category, measurement_count) 
        ...
        data_to_insert = [
            (
                ...
                float(row["avg_total_ozone"]), float(row["max_total_ozone"]),
                float(row["min_total_ozone"]), float(row["ozone_std"]),
        ...
        context.log.info(f"✅ Successfully loaded {len(df)} daily TOX records for Egypt")
        return "success"
...
@job
def nasa_daily_total_ozone_2022_pipeline():
    """Pipeline لبيانات تركيز الأوزون الكلي"""
    files = search_nasa_files_tox_2022()
    processed = files.map(process_single_file_tox)
    transformed = processed.map(transform_daily_tox)
    transformed.map(load_daily_tox_to_snowflake)
