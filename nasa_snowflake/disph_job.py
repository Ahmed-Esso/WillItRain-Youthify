# jobs/core_weather/disph_job.py
import pandas as pd
...
@op(out=DynamicOut(), name="search_files_disph")
def search_files_disph(context):
...
@op(name="process_file_disph")
def process_file_disph(context, granule):
    try:
        ...
        if VARIABLE not in ds:
            ...
            # Try common alternative names for displacement height
            alternatives = ["DISPH", "Z0M", "ROUGHNESS", "Z0", "SURFACE_ROUGHNESS", "Z0H"]
            ...
            context.log.info("No displacement height variable found, returning empty DataFrame")
            return pd.DataFrame()
        
        # If DISPH is found, process normally
        ...
        # DISPH is displacement height, usually in meters
        # No unit conversion needed typically
        
        ds.close()
        context.log.info(f"Processed {len(df)} DISPH data points")
        return df[["time", VARIABLE]]
    except Exception as e:
        context.log.error(f"Error processing DISPH file: {e}")
        return pd.DataFrame()

@op(name="transform_daily_disph")
def transform_daily_disph(context, df):
    if df.empty: 
        context.log.info("No data to transform for DISPH")
        return pd.DataFrame()
...
@op(name="load_to_snowflake_disph")
def load_to_snowflake_disph(context, df):
    if df.empty: 
        context.log.info("No DISPH data to load")
        return
    ...
    try:
        ...
        conn.commit()
        context.log.info(f"Successfully loaded {len(rows)} DISPH rows into {table_name}")
        
    except Exception as e:
        context.log.error(f"Error loading DISPH data to Snowflake: {e}")
        conn.rollback()
        raise e
    finally:
        cur.close()
...
@job
def disph_job():
    files = search_files_disph()
    processed = files.map(process_file_disph)
    transformed = processed.map(transform_daily_disph)
    transformed.map(load_to_snowflake_disph)
