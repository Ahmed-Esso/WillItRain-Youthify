...
VARIABLES = ["T2MWET"]  # درجة حرارة الهواء الرطبة على ارتفاع 2 متر
...
# ==========================
# DAGSTER OPS - DAILY AVERAGE FOR T2MWET
# ==========================
...
@op
def process_single_file_t2mwet(context, granule) -> pd.DataFrame:
    """
    معالجة ملف واحد وحساب المتوسط اليومي لـ T2MWET
    """
    try:
        ...
        if not all_daily_data:
            context.log.warning(f"⚠️ No T2MWET variable found in file")
            return pd.DataFrame()
        
        # نجمع كل البيانات اليومية
        combined = pd.concat(all_daily_data, ignore_index=True)
        context.log.info(f"✅ Processed {len(combined)} daily T2MWET records from file")
...
@op
def transform_daily_t2mwet(context, df: pd.DataFrame) -> pd.DataFrame:
    """
    تحويل البيانات اليومية لـ T2MWET للتخزين في Snowflake
    """
    if df.empty:
        return df
    
    context.log.info(f"🔄 Transforming {len(df)} daily T2MWET records...")
    ...
    daily_summary = (
        ...
        .agg({
            'T2MWET': 'mean',  # متوسط درجة حرارة الهواء الرطبة
    ...
    result = daily_summary[[
        ...
        "avg_wetbulb_temp", "measurement_count"
    ]]
    
    context.log.info(f"✅ Transformed to {len(result)} daily T2MWET summary records")
...
@op
def load_daily_t2mwet_to_snowflake(context, df: pd.DataFrame):
    """
    تحميل بيانات درجة حرارة الهواء الرطبة اليومية لـ Snowflake
    """
    if df.empty:
        context.log.warning("⚠️ Empty dataframe - skipping load")
        return "skipped"
    
    context.log.info(f"📤 Loading {len(df)} daily T2MWET records to Snowflake...")
    ...
    try:
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS NASA_DAILY_WETBULB_TEMP (
                date DATE,
                year INT,
                month INT,
                day INT,
        ...
        insert_query = """
            INSERT INTO NASA_DAILY_WETBULB_TEMP 
            (date, year, month, day, day_of_year, day_name, season, variable, avg_wetbulb_temp, measurement_count) 
        ...
        context.log.info(f"✅ Successfully loaded {len(df)} daily T2MWET records")
...
# ==========================
# DAGSTER JOB - DAILY WETBULB TEMP PIPELINE 2022
# ==========================

@job
def nasa_daily_wetbulb_2022_pipeline():
    """
    Pipeline لمعالجة بيانات درجة حرارة الهواء الرطبة اليومية لسنة 2022
    """
    # بحث عن الملفات لسنة 2022
    files = search_nasa_files_2022()
    
    # معالجة كل ملف وحساب المتوسطات اليومية لـ T2MWET
    processed = files.map(process_single_file_t2mwet)
    
    # تحويل البيانات اليومية
    transformed = processed.map(transform_daily_t2mwet)
    
    # تحميل البيانات اليومية لـ Snowflake
    transformed.map(load_daily_t2mwet_to_snowflake)
