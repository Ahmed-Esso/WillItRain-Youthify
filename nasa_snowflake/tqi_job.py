...
@op(out=DynamicOut())
def search_tqi_files(context):
    """البحث عن ملفات TQI في MERRA-2 لسنة 2022 للإسكندرية"""
...
@op
def process_tqi_file(context, granule):
    """معالجة ملف واحد واستخراج TQI للإسكندرية"""
    try:
...
@op
def transform_tqi_daily(context, df: pd.DataFrame):
    """حساب الإحصائيات اليومية لـ TQI"""
    if df.empty:
        return pd.DataFrame()
...
@op
def load_tqi_to_snowflake(context, df: pd.DataFrame):
    ...
    conn.commit()
    cur.close()
    context.log.info(f"✅ تم تحميل {len(df)} يوم لـ TQI")

@job
def tqi_job():
    """Job كامل لمعالجة TQI للإسكندرية 2022"""
    files = search_tqi_files()
    processed = files.map(process_tqi_file)
    transformed = processed.map(transform_tqi_daily)
    transformed.map(load_tqi_to_snowflake)
