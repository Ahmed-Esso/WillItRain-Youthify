...
def transform_daily_tql(context, df: pd.DataFrame) -> pd.DataFrame:
    """
    تحويل البيانات اليومية لـ TQL للتخزين في Snowflake
    """
    if df.empty:
        return df
    ...
    daily_summary = (
        ...
        .agg({
            'TQL': 'mean',  # متوسط إجمالي عمود البخار المائي
    ...
    daily_summary.rename(columns={
        'TQL': 'avg_total_precipitable_water',
...
def load_daily_tql_to_snowflake(context, df: pd.DataFrame):
    """
    تحميل بيانات إجمالي عمود البخار المائي اليومية لـ Snowflake
    """
    if df.empty:
    ...
    try:
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS NASA_DAILY_TOTAL_PRECIPITABLE_WATER (
                ...
                season STRING,
                variable STRING,
                avg_total_precipitable_water FLOAT,
                max_total_precipitable_water FLOAT,
                min_total_precipitable_water FLOAT,
                tpw_std FLOAT,
        ...
        data_to_insert = [
            (
                ...
                row["season"],
                row["variable"], 
                float(row["avg_total_precipitable_water"]),
                float(row["max_total_precipitable_water"]),
                float(row["min_total_precipitable_water"]),
                float(row["tpw_std"]),
                row["humidity_category"],
...
# ==========================
# DAGSTER JOB - DAILY TOTAL PRECIPITABLE WATER PIPELINE 2022
# ==========================

@job
def nasa_daily_total_precipitable_water_2022_pipeline():
    """
    Pipeline لمعالجة بيانات إجمالي عمود البخار المائي اليومية لسنة 2022 لكل مصر
    """
    # بحث عن الملفات لسنة 2022
    files = search_nasa_files_tql_2022()
    
    # معالجة كل ملف وحساب المتوسطات اليومية لـ TQL
